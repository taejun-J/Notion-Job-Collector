...
- 공개 채용 달력은 API 조회 범위를 고려해 현재 월을 15일 단위로 나누어 요청하고, 직무 분류 API는 실행당 한 번 요청합니다. 상세 공고는 반복 조회하지 않습니다.
- 현재 모집 중인 공고만 남깁니다.
- 수집 결과는 지원 마감일이 빠른 순으로 정렬하고, 마감일이 같으면 적합도가 높은 순으로 정렬합니다.
- 자소설닷컴 직무 분류가 `IT·인터넷` 하위이거나, 공고명·공개 직무명에 독립된 단어 `IT`가 있으면 모두 수집합니다.
